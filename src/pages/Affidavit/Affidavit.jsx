...
function Affidavit() {
    ...
    useEffect(() => {
        const getFormats = async () => {
            try {
                const response = await fetchAffidavitFormats();
                if (response.success) {
    ...
    const renderUploadContent = () => (
        <div className="affidavit-card-content">
            ...
            <Button appearance="primary" size="large" className="full-width-btn" disabled={loading}>
                Upload Affidavit Format
            </Button>
            <Text align="center">Upload an existing document to use as a template for your new affidavit.</Text>
        </div>
    );
        ...
        <div className="affidavit-card-content">
            {loading ? (
                ...
                <>
                    ...
                    <Text align="center">Choose from our library of expertly drafted affidavit templates.</Text>

                    <Button
                        appearance="primary"
                        style={{ marginTop: '0.5rem', width: '100%' }}
                        disabled={!selectedFormat}
                        onClick={() => navigate(`/affidavit/${selectedFormat}`)}
                    >
                        Proceed
                    </Button>
                </>
    ...
    return (
        <main className="main-content">
            <div className="hero-section">
                <Title1 as="h1">Select Affidavit Option</Title1>
                <Text className="hero-subtitle">Choose how you want to create your new affidavit.</Text>
            </div>
