/**
 * Legacy rent-agreement routes — mirrors legacyAffidavitRoutes.
 * Serves the RentAgreement / RentAgreementForm frontend pages via /api/rent-agreements/*.
 * Reads template files from file_storage/rent-agreements/ and metadata from data/rent-agreement/rent-agreements.json.
 */
...
const GENERATED_DIR = path.join(BACKEND_ROOT, 'file_storage', 'generated_rent_agreements');

if (!fs.existsSync(GENERATED_DIR)) {
  fs.mkdirSync(GENERATED_DIR, { recursive: true });
}

interface RentAgreementFormat {
  id: string;
  title: string;
  description: string;
  category: string;
...
async function getAllFormats(): Promise<RentAgreementFormat[]> {
  const data = await fsp.readFile(DATA_PATH, 'utf-8');
  return JSON.parse(data) as RentAgreementFormat[];
}

async function getFormatById(id: string): Promise<RentAgreementFormat> {
...
async function getFormatWithContent(id: string): Promise<RentAgreementFormat & { content: string }> {
...
export const legacyRentAgreementRoutes = router;
